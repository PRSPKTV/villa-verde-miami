...
export default function CTASection() {
  const { content, loading } = useSiteContent('cta');
  ...
  return (
    <section ref={sectionRef} className="relative py-32 md:py-40 px-4 md:px-8 overflow-hidden">
      <div className="absolute inset-0 z-0">
        <img
          src={content?.backgroundImage || "https://a0.muscache.com/im/pictures/37849b8f-1280-4936-9539-a448172b3c6b.jpg"}
          alt="Villa Verde"
      ...
      <div className="relative z-10 max-w-3xl mx-auto text-center">
        <h2 className="cta-anim font-heading italic text-4xl md:text-6xl lg:text-7xl text-gold-500 mb-6 leading-[1.1]">
          {content?.heading || 'Your Little Havana Oasis Awaits'}
        </h2>
        <p className="cta-anim text-cream-200/80 font-body text-xl mb-10 max-w-lg mx-auto">
          {content?.subtitle || 'Experience the beauty of Little Havana from your own private tropical retreat.'}
        </p>
        <div className="cta-anim">
          <Button to={content?.buttonLink || '/properties'} size="lg">
            {content?.buttonText || 'Explore Properties'} <ArrowRight size={18} />
          </Button>
        </div>
      </div>
    </section>
